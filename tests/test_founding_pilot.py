...
from services.intake.messaging import PILOT_HEADLINE, intake_messaging_blocks
...
def test_quiet_prospect_can_queue_under_pilot(pilot_on):
...
def test_consultant_still_rejected(pilot_on):
    title = "CMMC AMA"
...
def test_adaptive_threshold_targets_nonzero_queue(pilot_on):
    candidates = [
...
def test_pilot_telemetry_emits(pilot_on):
    from services.memory.telemetry import load_telemetry

    emit_intake_event("pilot_upload_started", metadata={"test": True})
    # emit_intake_event writes subsystem="intake" (post-pilot rebrand);
    # the event_type retains the historical name for log continuity.
    rows = load_telemetry(limit=30, subsystem="intake")
    assert any(r.get("event_type") == "pilot_upload_started" for r in rows)


def test_acquisition_dashboard_includes_founding_pilot():
    from services.acquisition.orchestration import get_operator_dashboard

    dash = get_operator_dashboard()
    assert "founding_pilot" in dash
    assert dash["founding_pilot"].get("active") is True


def test_control_html_intake_strip(client):
    """Operator cockpit must render the intake strip in control.html.
    Post-rebrand: class is 'intake-strip' (was founding-pilot-strip)."""
