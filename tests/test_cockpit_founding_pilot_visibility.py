"""Cockpit surfaces founding pilot paperwork without scrolling."""
...
def test_control_html_cockpit_paperwork_visibility(client):
    ...
    (intake-* instead of founding-pilot-*)."""
    ...
    assert "founding-pilot-intake-insights" not in text
    assert "fb-queue-atlas" in text
    assert "cockpit-intake.js" in text
    assert "/api/operator/intake/queue" in text


def test_topology_attention_when_founding_pilot_pending(client, fb_env, anon_client):
    anon_client.post(
        "/api/intake/upload",
    ...
    assert "PAPERWORK" in attention.upper() or "founding pilot" in attention.lower()


def test_founding_pilot_queue_endpoint_ok(client, fb_env, anon_client):
    anon_client.post(
        "/api/intake/upload",
