"""Founding pilot hooks for acquisition probability (avoids circular imports)."""
...
def deployable_intent(
    ...
    *,
    ...
    if soft_score >= 42 and has_personal_need and predator_raw < 12:
        return True
    if _pilot() and intent == "UNKNOWN" and soft_score >= 35 and has_personal_need and predator_raw < 10:
        return True
    if operational_pressure_score >= 32 and has_personal_need and predator_raw < 15:
        return True
    if _pilot() and operational_pressure_score >= 38 and predator_raw < 12:
        return True
    return False
...
def intent_passes_prey_gate(intent: str, *, soft_score: int, prob: Dict[str, Any]) -> bool:
    if intent in ("SEEKING_HELP", "VENTING_OR_OVERWHELMED"):
        return True
    if _pilot() and intent == "UNKNOWN" and soft_score >= 30 and prob.get("has_operational_need"):
        return True
    if soft_score >= 38 and prob.get("has_operational_need") and int(prob.get("operational_entanglement_score", 0)) >= 25:
        return True
    if _pilot() and soft_score >= 32 and int(prob.get("paperwork_likelihood_score", 0)) >= 28:
        return True
    return False


def passes_founding_pilot_prey_gate(
    qualification: Dict[str, Any],
    classification: Dict[str, Any],
    *,
    min_prey_score: int,
) -> bool:
    """Founding pilot primary gate — operational burden over distress."""
