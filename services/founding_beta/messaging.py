"""Reusable Founding Beta copy blocks — upload-first, low pressure."""
...
def beta_messaging_blocks() -> Dict[str, Any]:
    return {
...
def beta_outreach_snippet(*, include_route: bool = False, route_url: str = "") -> str:
    """Short Reddit/public paste block for founding beta validation."""
    parts: List[str] = [
        BETA_HEADLINE,
        "",
        BETA_INTRO,
    ...
    if include_route and route_url:
        parts.extend(["", f"If you want to try the beta intake: {route_url}"])
    return "\n".join(parts)
