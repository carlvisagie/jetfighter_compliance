"""Safe learning_state updates for intake pipeline operational events."""
...
def record_intake_learning(
    ...
    *,
    ...
    """Bump intake pipeline counters without heavy memory graph imports."""
    try:
        state = _load()
        ...
        if extra:
            intake_bucket = state.setdefault("intake", {})
            if isinstance(intake_bucket, dict):
                intake_bucket.update(
                    {k: v for k, v in extra.items() if k in ("last_intake_id", "queue_depth")}
                )
