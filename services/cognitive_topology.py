...
def _learning_signal() -> Dict[str, float]:
    raw = read_text_bounded(_LEARNING, max_bytes=64 * 1024)
    if not raw.strip():
        return {"health": 0.6, "activity": 0.15, "confidence": 0.5, "pressure": 0.1}
    try:
        data = json.loads(raw)
    except json.JSONDecodeError:
        return {"health": 0.45, "activity": 0.1, "confidence": 0.35, "pressure": 0.25, "anomaly": True}
    trends = data.get("trends") if isinstance(data, dict) else {}
    n = len(trends) if isinstance(trends, dict) else 0
    activity = _clamp(n / 12.0)
    return {
        "health": _clamp(0.5 + activity * 0.4),
        "activity": activity,
        "confidence": _clamp(0.45 + activity * 0.35),
        "pressure": _clamp(0.12 if n else 0.22),
        "anomaly": False,
    }
