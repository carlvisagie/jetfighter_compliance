...
The composite per-company endpoint, `GET /api/operator/vio/company/{intake_id}`, remains in place and now powers the cockpit drill-down. It returns every uploaded document (with view/download URLs), every generated document, every missing document, the evidence intelligence summary, all identifiers, and a derived `findings` block that surfaces extraction failures, stale payment links, on-disk file mismatches, ghost intakes, unindexed files, and confirmation-needed states. The Level 2 immersive landscape (the per-company recursive visual that the operator will see when they click a Level 1 trace) is the explicit next build; a clean placeholder is shown today.
