...
The `founding_pilot` name appears in ~70 files. **Almost all of it is canonical business logic**, not stale residue:

- `services/acquisition/founding_pilot_mode.py` — the prey-gate that lowers acquisition thresholds during early discovery.
- `services/intake/mode.py` — the `is_intake_mode()` helper that reads `KYC_FOUNDING_PILOT_MODE`.
...
The organism's `pilot_residue_scan` correctly flags these as **AMBER** — "active source code references the deprecated `founding_pilot` name." That is truth, not noise.

## Why it isn't done yet

A previous session looked at this and found:

> structural business logic and is not simply "pilot" residue to be purged blindly

Renaming touches:
- 1 Python file rename (`services/acquisition/founding_pilot_mode.py`)
- ~15 active services modules with calls / imports
- 4 UI/JS/CSS files with strings the operator sees
- 1 env-var name (`KYC_FOUNDING_PILOT_MODE`)
- 1 storage directory name (`data/founding_pilot/` — production-data sensitive)
- ~20 test files
- 5 doctrine docs
...
| Old | New |
|---|---|
| `services/acquisition/founding_pilot_mode.py` | `services/acquisition/expanded_discovery_mode.py` |
| `KYC_FOUNDING_PILOT_MODE` env var | `KYC_EXPANDED_DISCOVERY_MODE` (with the old name as a deprecation alias for one release) |
| `is_intake_mode()` / `is_founding_pilot_mode()` | `is_expanded_discovery_mode()` |
| `passes_founding_pilot_prey_gate(...)` | `passes_expanded_discovery_prey_gate(...)` |
| `pilot_residue_scan` check | `legacy_mode_naming_scan` check |
| `KYC_FOUNDING_PILOT_DOCTRINE.md` | `EXPANDED_DISCOVERY_DOCTRINE.md` |
| UI labels "Founding Pilot" / "Pilot" | "Expanded Discovery" / "Early Discovery" |
| `data/founding_pilot/` directory | **DO NOT RENAME** — production-data path. Read-compatibility shim only. |
...
9. **DO NOT** rename `data/founding_pilot/` — that path is a storage convention with possible historical artifacts. Read-side may dual-read both directories transparently.
