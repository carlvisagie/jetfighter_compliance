# Founding Beta intake pipeline soak verification
# Usage: powershell -File scripts/verify_founding_beta_pipeline.ps1 [-BaseUrl URL] [-OpsPassword PASS]
...
function Upload-Files([string[]]$names, [hashtable]$extra) {
    ...
    $body.Write($enc.GetBytes("--$boundary--$LF"), 0, $enc.GetByteCount("--$boundary--$LF"))
    $resp = Invoke-WebRequest "$BaseUrl/api/founding-beta/upload" -Method POST -WebSession $session `
...
foreach ($path in @(
        "/api/operator/founding-beta/queue",
        "/api/operator/founding-beta/diagnostics",
        "/api/operator/founding-beta/reconcile",
        "/api/operator/founding-beta/retention-check/$iid",
        "/api/operator/founding-beta/intake/$iid/audit"
    )) {
    try {
...
try {
    $rc = Invoke-RestMethod "$BaseUrl/api/operator/founding-beta/retention-check/$iid" -WebSession $session
